...
# Functions
- [ascii_board(frame* parent, const std::string& path, const std::string& name_id, const std::string& special_operation = "none", int lines_count = 0, bool start_logging=false, const std::string& logging_file_path="ascii_board.log")](#ascii_board)
- [void clear_tile(int row, int column)](#clear_tile)
- [void clear_row(int row)](#clear_row)
- [void clear_column(int column)](#clear_column)
- [void clear_all()](#clear_all)
- [std::string get_tile(int row, int column)](#get_tile)
...
- [void build()](#build)
- [action_tile get_action_tile(int row, int column)](#get_action_tile)
- [bool configuration_activated(const std::string& name_id, int row, int column)](#modify_configuration)
- [void modify_configuration(const std::string& target_name_id, const std::string& modification_name_id)](#modify_configuration)
- [int load_board_translation(const std::string& name_id, const std::string& path, bool load_metadata = false)](#load_board_translation)
- [void use_translation(const std::string& name_id, bool swap_metadata = false)](#use_translation)
- [void set_lines_count(int lines_count)](#set_lines_count)
- [void set_top_display_index(unsigned int index)](#set_top_display_index)
- [void set_left_display_index(unsigned int index)](#set_left_display_index)
- [void scroll_up(unsigned int amount=1)](#scroll_up)
- [void scroll_down(unsigned int amount=1)](#scroll_down)
...
- [tile_configuration](#struct-tile_configuration)
- [board_configuration](#struct-board_configuration)

# Member structures
- [action_tile_board_section](#struct-action_tile_board_section)
- [sub_tile_configuration](#struct-sub_tile_configuration)
- [action_tile](#struct-action_tile)
...
**Parameters:**
1. frame* **parent** - the frame that the board should be placed in
2. const std::string& **path** - absolute or relative path to the board configuration file (the board configuration file is generated using build_board_config)
3. const std::string& **name_id** - name to uniquely identify default board incase board translation are used
4. const std::string& **special_operation** - optional parameter used to define how the ascii_board widget should be placed in the frame (leaving blank will result in the widget being placed to the right of the last widget, "new line" will result in the widget being placed below on a new line, and "merge" will result in the widget being placed below the last widget but still in the same row)
5. int **lines_count** - number of lines to be displayed in the console at a time (if 0, there is no limit and if negative, the number of lines will be dynamically determined based on total number of lines in the console minus the negative value)
6. bool **start_logging** - if set to true, a log file will be created where logs from ascii_board member functions will be listed
7. const std::string& **logging_file_path** - set the absolute or relative path to the logging file (if not specified, the log file will be created as ascii_board.log)
...
**Parameters:**
1. int **row** - row coordinate where the tile should be cleared
2. int **column** - column coordinate where the tile should be cleared
...
**Parameters:**
1. int **row** - row coordinate where all tiles should be cleared
...
**Parameters:**
1. int **column** - column coordinate where all tiles should be cleared
...
**Parameters:**
1. int **row** - row coordinate of tile that should have its value returned
2. int **column** - column coordinate of tile that should have its value returned
...
**Parameters:**
1. int **row** - row coordinate of tile that should have its value set
2. int **column** - column coordinate of tile that should have its value set
3. std::string **value** - value that the tile should contain
...
**Parameters:**
1. int **row** - row coordinate where all tiles should be set to desired value
2. int **column** - column coordinate where all tiles should be set to desired value
3. unsigned int **character_index** - character's index of board tile's value
...
**Parameters:**
1. board_configuration **configuration** - structure holding the board configuration's data
...
4. const std::string& **value** - value that board tile should be set to when configuration is activated
5. char **ignore_character** - defines character in value that should be ignored: whatever is in the tile already at the indexes of ignore characters should remain unchanged
6. int **foreground_format** - color code defining the color to be applied to the foreground
...
**Parameters:**
1. const std::string& **path** - absolute or relative path to the file containing a configuration's value
2. const std::string& **name_id** - name that also acts as the id of the configuration
...
**Parameters:**
1. int **row** - row coordinate where the action_tile data structure should be retrieved
2. int **column** - column coordinate where the action_tile data structure should be retrieved
...
**Parameters:**
1. const std::string& **name_id** - name of the configuration to look for on the action tile
2. int **row** - row coordinate of the action tile to look for the configuration
3. int **column** - column coordinate of the action tile to look for the configuration
...
**Parameters:**
1. const std::string& **target_name_id** - name of the configuration to be searched for where it is activated on the board
2. const std::string& **modification_name_id** - name of the configuration to be activated on action tiles where the configuration identified by target_name_id is activated
...
**Parameters:**
1. const std::string& **name_id** - name identifing the translation
2. const std::string& **path** - absolute or relative path to the file containing a board configuration
3. bool **load_metadata** - will attempt to load metadata from the board config file if set to true, otherwise, metadata will not be loaded
...
**Parameters:**
1. const std::string& **name_id** - name identifing the translation to be used
2. bool **swap_metadata** - will swap current metadata with the metadata of the new translation if true, otherwise, it will keep the current metadata
...
**Parameters:**
1. unsigned int **lines_count** - number of lines displayed in the console at once
...
**Parameters:**
1. unsigned int **index** - index defining the top visible line of the board
...
**Parameters:**
1. unsigned int **index** - index defining the left visible column of the board
...
**Parameters:**
1. unsigned int **amount** - number of lines to scroll up (will do nothing if it would otherwise scroll beyond top side of board)
...
**Parameters:**
1. unsigned int **amount** - number of lines to scroll down (will do nothing if it would otherwise scroll beyond bottom side of board)
...
**Parameters:**
1. unsigned int **amount** - number of columns to scroll left (will do nothing if it would otherwise scroll beyond left side of board)
...
1. unsigned int **amount** - number of columns to scroll right (will do nothing if it would otherwise scroll beyond right side of board)
...
**Parameters:**
1. int **row** - row of tile that should be visible next time board is displayed
2. int **column** - column of the tile that should be visible next time board is displayed
3. int **top_padding** - minimum number of lines that should be shown above the tile
...
1. int **row** - row coordinate where the tile should be set to desired value
...
1. int **board_start_row** = -1;
2. int **board_stop_row** = -1;
3. int **board_start_column** = -1;
4. int **board_stop_column** = -1;

**Value descriptions:**

1. int **board_start_row** - the row in the '\n' separated string holding the board where the tile begins
2. int **board_stop_row** - the row in the '\n' separated string holding the board where the tile ends
3. int **board_start_column** - the column in the '\n' separated string holding the board where the tile begins
4. int **board_stop_column** - the column in the '\n' separated string holding the board where the tile ends
